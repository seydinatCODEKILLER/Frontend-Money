...
export const alertApi = {
  ...
  getUserAlerts: async (
    ...
    if (!response.data.success) {
      throw new Error(
        response.data.message || "Erreur lors du chargement des alertes"
      );
    }
  ...
  markAlertAsRead: async (alertId: string): Promise<Alert> => {
    ...
    if (!response.data.success) {
      throw new Error(
        response.data.message || "Erreur lors du marquage de l'alerte"
      );
    }
    return response.data.data;
  },
  ...
  markAllAlertsAsRead: async (): Promise<void> => {
    ...
    if (!response.data.success) {
      throw new Error(
        response.data.message || "Erreur lors du marquage des alertes"
      );
    }
  },
  ...
  getAlertStats: async (): Promise<AlertStats> => {
    ...
    if (!response.data.success) {
      throw new Error(
        response.data.message || "Erreur lors du chargement des statistiques"
      );
    }
    return response.data.data;
  },
